...
func (m *MemoryBus) ReadBytes(pointer, n uint16) []byte {
	return m.Route(pointer).ReadBytes(pointer, n)
}

func (m *MemoryBus) Read(pointer uint16) byte {
	return m.Route(pointer).Read(pointer)
}

func (m *MemoryBus) StoreBytes(pointer uint16, bytes []byte) {
	m.Route(pointer).StoreBytes(pointer, bytes)
}

func (m *MemoryBus) Store(pointer uint16, val byte) {
