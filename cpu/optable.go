...
var ( // todo check table (especially loads)
	optable = [...]Instr{
		...
		jrnc(BitC), load(sp(), dx(16)), ldHl(nil, rx(A), false), inc16bit(sp()), inc8bit(mr(HL)), dec8bit(mr(HL)), load(mr(HL), dx(8)), scf, jrc(BitC), add16b(rx(HL), sp()), ldHl(rx(A), nil, false), dec16bit(sp()), inc8bit(rx(A)), dec8bit(rx(A)), load(rx(A), dx(8)), ccf,
