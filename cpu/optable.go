...
var ( // todo check table (especially loads)
	optable = [...]Instr{
		...
		load(mem(off(dx(8), 0xFF00)), rx(A)), pop(rx(HL)), load(mem(off(rx(C), 0xFF00)), rx(A)), invalid, invalid, push(rx(HL)), and(rx(A), dx(8)), rst(hc(0x20)), addSp, jp(mr(HL)), load(md(16), rx(A)), invalid, invalid, invalid, xor(rx(A), dx(8)), rst(hc(0x28)),
